    ...
    try:
        ...
        if provider == "deepseek":
            ...
            if response.status_code == 200:
                ...
                for line in response.text.strip().split('\n'):
                    try:
                        ...
                        if json_response.get('message', {}).get('content'):
                            ...
                            # Skip think tags and empty lines
                            if content in ['<think>', '</think>', '\n', '\n\n']:
                                continue
                ...
                print(f"Error from DeepSeek API: {response.text}", file=sys.stderr)
                return None
                
        elif provider in ["openai", "local", "azure"]:
